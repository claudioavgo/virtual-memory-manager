...
#define GET_PHYSICAL_ADDRESS(frame, offset) ((frame << OFFSET_BITS) | offset)
...
uint32_t page_table[NUMBER_OF_PAGES], next_available_TLB, next_available_frame;
uint8_t page_valid[NUMBER_OF_PAGES];
uint32_t frame_last_used[NUMBER_OF_FRAMES]; // Track the last used time of each frame
uint32_t current_time = 0; // Global time counter for LRU
...
uint32_t select_victim_frame() {
    // LRU
    uint32_t lru_frame = 0, min_time = UINT_MAX;
    ...
    for (size_t i = 0; i != NUMBER_OF_PAGES; ++i) {
        if (page_valid[i] && page_table[i] == lru_frame) {
            page_valid[i] = 0;
            break;
        }
    }
...
void handle_page_fault(uint32_t page_number) {
    ...
    fread(memory + frame_number * PAGE_SIZE, sizeof(int8_t), PAGE_SIZE, backing_storage);
    page_valid[page_number] = 1;
    frame_last_used[frame_number] = current_time++; // Update the time of last use
    ++page_fault_cnt;
}
...
int init(int argc, char **argv) {
    ...
    memset(frame_last_used, 0, sizeof(uint32_t) * NUMBER_OF_FRAMES);
